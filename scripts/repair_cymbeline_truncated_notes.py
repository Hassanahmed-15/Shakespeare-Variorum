#!/usr/bin/env python3
"""Repair truncated Cymbeline NV notes from IA witness (unset0000unse_m8h0)."""
...
JSON_PATH = ROOT / "Public/Data/cymbeline.json"
BACKUP = JSON_PATH.with_suffix(".json.pre_clip_repair.backup")
SITE_MIRROR = ROOT / "My Website/Public/Data/cymbeline.json"
PLAY = "Cymbeline"
...
PLAY_BLOCK = re.compile(
    r"ACT\s+[IVXLC\d]+\s*,\s*SC\.?\s*[IVXLC\divxlc]+\.?\]?\s*",
    re.I,
)
CYMB_HDR = re.compile(
    r"(?:THE\s+TRAGED(?:IE|ILZ|Y)\s+OF|CYMBELINE)\s*(?:\[ACT|\d{1,3})",
    re.I,
)
SCENE_HDR = re.compile(r"\[ACT\s+[^\]]+\]", re.I)
...
def is_truncated(note: str, folded_ia: str) -> bool:
    ...
    if is_unbalanced_parens(note):
        return True
    if folded_ia and is_witness_prefix(folded_ia, note):
        return True
    return False
...
def find_note_pos_cymb(ia: str, note: str) -> int:
...
def extract_from_ia_cymb(ia: str, pos: int, json_note: str) -> str:
    start = max(0, pos - 120)
    chunk = ia[start : start + 8000]
    m = re.search(r"(\d{1,3}\.\s*)?[\w .'\"-]+\]\s*[A-Z(]", chunk, re.I)
    if m and m.start() <= (pos - start) + 40:
        chunk = chunk[m.start() :]
    ext = _h4p2.truncate_footnote(deinterleave_cymb(chunk))
    if "]" in json_note:
...
def looks_contaminated(note: str) -> bool:
    return bool(
        ...
        and re.search(r"CYMBELINE|TRAGEDIE\s+OF", note, re.I)
    )


def is_fixed(note: str, folded_ia: str) -> bool:
        ...
        for line_data in scene_data.values():
            ...
            for i, note in enumerate(notes):
                ...
                if is_hyphen_artifact(note):
                    ...
                    if ext and not is_fixed(ext, folded_ia):
                        ext = None
                if ext is None:
                    pos = find_note_pos_cymb(ia, note)
                    if pos < 0:
                        stats["unresolved"] += 1
                        continue
                    ext = extract_from_ia_cymb(ia, pos, note)
                ...
                if not improved and is_fixed(ext, folded_ia) and len(ext) >= len(note) - 50:
                    improved = not is_hard_truncation(note) or is_fixed(ext, folded_ia)
                if improved:
                    if not dry_run:
                        notes[i] = ext
                    stats["repaired"] += 1
...
def main() -> int:
    ...
    stats = repair(data, ia_text, folded_ia, dry_run=args.dry_run)
    ...
    audit = ROOT / "validation/nv_cymbeline_repair.json"
    audit.write_text(
