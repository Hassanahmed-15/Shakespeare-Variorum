# Tail-verify repair cohort split

**Date:** 2026-07-10

nv_tail_verify_all_plays.json stores per-play aggregates only; workbook cohort scored per-note; untouched cohort derived by subtraction from corpus census totals.

**Excluded plays (1):** Troilus and Cressida

Repair manifest: `contractor_truncation_workbook.json` (927 entries, 89 excluded, 824 matched in current corpus)

## Two-group split

| Cohort | Notes | Pass | Fail | Pass % |
|--------|------:|-----:|-----:|-------:|
| Never in workbook | 22,270 | 21,905 | 365 | **98.36%** |
| Workbook-flagged (repair cohort) | 824 | 810 | 14 | **98.3%** |
| Full corpus (census) | 23,094 | 22,715 | 379 | **98.36%** |

## By workbook status

| Status | n | Pass % |
|--------|--:|-------:|
| complete | 734 | 98.91% |
| gap_marked | 9 | 44.44% |
| no_change_needed | 81 | 98.77% |

## Spliced only (`complete` + text changed)

| n | Pass | Fail | Pass % |
|--:|-----:|-----:|-------:|
| 734 | 726 | 8 | **98.91%** |

Auditable per-note cross-ref: `nv_tail_verify_repair_split_no_troilus.json` → `workbook_crossref`
